...
export class Bitrix24Client {
  ...
  private readonly MIN_REQUEST_INTERVAL = 500; // 500ms between requests (2 req/sec safe limit)
    ...
    while (this.requestQueue.length > 0) {
      ...
      if (request) {
        try {
          ...
          console.error('[Bitrix Queue] Request failed within queue processing', err);
        }
      }
    }
  ...
  private async getCachedOrFetch<T>(key: string, fetchFn: () => Promise<T>, ttl: number = this.CACHE_TTL): Promise<T> {
    ...
    if (cached && cached.expiry > now) {
      // console.log(`[Bitrix Cache] Hit: ${key}`);
      return cached.data;
    }

    // console.log(`[Bitrix Cache] Miss: ${key}`);
